...
- [SA-MP Rich Presence](#sa-mp-rich-presence)
  ...
  - [Установка](#установка)
  - [Интеграция с SPC Integration API](#интеграция-с-spc-integration-api)
  ...
  - [Для разработчиков серверов](#для-разработчиков-серверов)
    - [Как внедрить SPC Integration на вашем сервере:](#как-внедрить-spc-integration-на-вашем-сервере)
...
- Клиент SA-MP (если у вас его нет: [clients-samp](https://github.com/spc-samp/clients-samp)).
...
## Интеграция с SPC Integration API

**SA-MP Rich Presence** значительно улучшает отображение информации в Discord благодаря интеграции с **SPC Integration API**. Вместо использования только базовых данных запроса, **ASI** теперь напрямую взаимодействует с этой API для получения визуальной и социальной информации о сервере. Чтобы узнать больше о **SPC Integration**, посетите официальный репозиторий: [spc-samp/spc-integration](https://github.com/spc-samp/spc-integration).
...
2. **Запрос к API:** **ASI** отправляет запрос `GET` на конечную точку **SPC Integration API**, включая отформатированные IP и порт. Пример того, как URL запроса строится внутри:
   ```cpp
...
3. **Обработка ответа JSON:** API возвращает ответ в формате JSON. **ASI** использует библиотеку [nlohmann/json](https://github.com/nlohmann/json) для разбора этого ответа и извлечения соответствующих данных.
   ```json
   // Пример структуры ответа SPC Integration API
   {
       "success": true,
       "data": {
           "public": {
...
Интеграция вашего сервера с [SPC Integration](https://github.com/spc-samp/spc-integration) приносит ряд прямых и косвенных преимуществ, которые повышают видимость и вовлеченность вашего сообщества SA-MP. Если вы разработчик сервера, рассмотрите стратегические преимущества:
...
3. **Упрощенное обновление контента:**
   - При использовании [SPC Integration](https://github.com/spc-samp/spc-integration) любое обновление логотипа вашего сервера или списка социальных сетей, настроенных в плагине [SPC Integration](https://github.com/spc-samp/spc-integration) **(на стороне сервера)**, автоматически отобразится для игроков, использующих **ASI**, без необходимости обновления **ASI**. Это гарантирует, что информация всегда актуальна.

### Как внедрить SPC Integration на вашем сервере:

1. **Плагин SPC Integration:** Вашему серверу потребуется установить и правильно настроить плагин [SPC Integration](https://github.com/spc-samp/spc-integration). Этот плагин является мостом, который отправляет информацию о вашем сервере в список данных **SPC**, который затем используется **SPC Integration API** для предоставления публичной информации.
2. **Настройка:** Следуйте инструкциям в репозитории плагина [SPC Integration](https://github.com/spc-samp/spc-integration) для настройки:
...
Интеграция вашего сервера с [SPC Integration](https://github.com/spc-samp/spc-integration) не только улучшает опыт ваших игроков, но и открывает двери для значительного роста и вовлеченности вашего сообщества.
...
- **`Main_Update_Thread`**: Душа **ASI**, работающая в собственной нити.
   ...
   DWORD WINAPI Main_Update_Thread(LPVOID) {
       ...
       while (is_running && v_server_manager && v_discord_manager) {
           v_server_manager->Update(); // Запрос сервера, API SPC и т.д.
...
`Server_Manager` — это оркестратор данных, управляющий состоянием подключения, запросами к серверу и **SPC Integration API**, а также решающий, какие данные должны отображаться и когда.
...
Класс `URL_Manager` управляет коммуникациями HTTPS с **SPC Integration API**, центральным элементом для новых визуальных и социальных функций **Discord Rich Presence**.
...
├── url_manager.hpp          # Определение менеджера URL для SPC Integration API
...
Этот файл посвящен защите конфиденциальной информации, такой как **Discord Application ID** и адреса **SPC Integration API (пока)**, от легкого обратного проектирования.
...
2. **Для серверов:**
   ...
   - **Консистентные данные:** Благодаря интеграции с **SPC Integration API** отображение информации унифицировано и обновляется централизованно, гарантируя, что бренд и контакты вашего сервера всегда актуальны.
